...
<div class="page-content-wrapper">
  ...
  <div class="product-description">
    ...
    <div class="product-title-meta-data bg-white py-3">
      <div class="container d-flex justify-content-between" @if($qty == 0) style=" filter: opacity(50%); -webkit-filter: opacity(50%); "@endif>
        <div class="p-title-price">
          ...
          <p class="sale-price mb-0">{{ $shopProduct->product->unit_price }}€<span>{{$shopProduct->discount_price}}€</span><small> ({{ $shopProduct->product->unit_price*1000 }} YummyCoin)</small></p>
        </div>
        <div class="p-wishlist-share" @if($qty == 0) style=" filter: opacity(50%); -webkit-filter: opacity(50%); "@endif>
          <form class="cart-form" action="{{ route('checkout.store') }}" method="POST">
            @csrf
            <input type="hidden" value="{{ $shopProduct->id }}" name="shop_product_id">
            @if($qty != 0)
            <button class="btn btn-lg btn-danger cartProtect mr-1" type="submit" data-shop_id="{{ $shopProduct->shop_id }}"> <span class="text">Réserver</span></button>
            @endif
          </form>
        </div>
      </div>
    ...
    <div class="p-specification bg-white" style="padding: 0.0rem 0.3rem;">
      <div class="container">
        <div class="coupon-text-wrap d-flex align-items-center">
          ...
          <p class="ps-2 mb-0">Disponible de {{\Carbon\Carbon::createFromFormat('H:i:s',$shopProduct->hdispoa)->format('H:i')}} à {{\Carbon\Carbon::createFromFormat('H:i:s',$shopProduct->hdispob)->format('H:i')}}</p>
        </div>
      </div>
    </div>
    @endif
    <ul class="page-nav ps-0 p-specification mb-3">
      <li><a href="{{ route('shop.show', $shopProduct->shop->slug) }}"><div class="coupon-text-wrap d-flex align-items-center">
    ...
    <div class="cart-form-wrapper bg-white mb-3 py-3">
      <div class="container">
        <form class="cart-form" action="{{ route('checkout.store') }}" method="POST">
          ...
          <input type="hidden" value="{{ $shopProduct->id }}" name="shop_product_id">
          @if($qty != 0)
          <button  class="btn btn-lg btn-danger cartProtect btn-block mr-1" type="submit" data-shop_id="{{ $shopProduct->shop_id }}"> <span class="text">Réserver</span> <i class="lni lni-package"></i> </button>
          @else
            <button class="btn btn-lg btn-dark cartProtect btn-block mr-1" type="submit" data-shop_id="{{ $shopProduct->shop_id }}" disabled> <span class="text">Aucun panier disponible</span></button>
          @endif
        </form>
      </div>
    </div>
