        ...
        <div class="section-body">
            <div class="row">
                <div class="col-12 col-md-12 col-lg-12">
                    <div class="card">
                        <form action="{{ route('admin.souscategorie.update', $subcategory->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="card-body">
                                <div class="form">
                                    ...
                                    <div class="form-group col">
                                        ...
                                        <select name="location_id" class="form-control">
                                            @foreach($locations as $loc)
                                                <option {{$loc->id == $subcategory->location_id ? 'selected':''}} value="{{ $loc->id }}">{{$loc->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
